...
func CommandUsage() {
	fmt.Fprintf(os.Stderr, "Usage: mimic local [OPTION]...\n")
...
func (c *Command) Run() {
	...
	if len(templateNames) > 0 {
		for _, templateName := range templateNames {
			sourcePaths = append(sourcePaths, filepath.Join(LocalTemplatesDirectoryPath, templateName))
		}
	}

	if len(sourcePaths) == 0 {
		sourcePaths = append(sourcePaths, LocalTemplatesDirectoryPath)
	}
	...
	lastSourcePath := LocalTemplatesDirectoryPath

	if value, err := c.config.SourcePath.Last(); err == nil {
		lastSourcePath = value
	}

	if len(templateNames) == 0 && strings.HasSuffix(lastSourcePath, LocalTemplatesDirectoryPath) {
